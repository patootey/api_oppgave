...
def default(buttonA, buttonB, label1, label2):
    buttonA.button.config(
        buttonA.settings if buttonA.clicked == True else buttonA.command_config
    )
...
def main(root):
    ...
    button1 = ge.Button(
        frame, {"text": "Søk med Land", "bg": "white"}, command_config=({"bg": "green"})
    )
    button1.button.grid(row=1, column=1)
    button2 = ge.Button(
        frame,
        {"text": "Søk med valuta", "bg": "white"},
        command_config=({"bg": "green"}),
        command=lambda: default(button1, button2, name_label1, name_label2),
    )
    ...
    search = ge.Button(
        frame,
        {"text": "Start Søk"},
        command=lambda: rp.results(
            root,
            name1_entry.get(),
            name2_entry.get(),
            name3_entry.get(),
            button1.clicked,
            start_date__entry.get(),
            end_date_entry.get(),
        ),
    )
